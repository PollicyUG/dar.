"""
A program that takes a policy question from a user, surveys open access research articles,
and uses natural language processing to determine if a study found evidence 'for' or 'against'
a given policy consequence.
...
# target words that we expect in an abstract of an article stating results of a study
claim_words = ['result', 'results', 'show', 'shows', 'showed', 'shown', 'find', 'finds', \
			   'findings', 'found', 'suggest', 'suggests', 'suggested', 'evidence', 'argue',\
			   'argues', 'argued', 'establish', 'establishes', 'established', 'conclude',\
			   'concludes', 'concluded', 'confirm', 'confirms', 'confirmed', 'points to', 
			   'indicate', 'indicates', 'indicated', 'view', 'see', 'seen', 'saw', 'effect' \
			   'effects', 'impact', 'cause', 'causal', 'significant', 'lead', 'leads', 'leads to', 'lead to']

# I removed sentiment analysis because issues with the opinion lexicon were breaking the application
# This could change once an appropriate fix is found
"""
# words that are used to determine sentiment
neg_words = opinion_lexicon.negative()
pos_words = opinion_lexicon.positive()
opinion_words = pos_words + neg_words
"""

# get user's question, clean it, and parse it for keywords
def process_question(question):
	# remove punctuation and get individual lower case words
	question_words = question.translate(str.maketrans('','',string.punctuation)).lower().split()
	# get the keywords
	keywords = [w for w in question_words if not w in stop_words]

	# get the lemmas for the keywords (we are not using these yet but could do so later)
...
# task to retrieve articles from DOAJ in json format
...
# task to retrieve articles from Crossref in json format	
...
def get_Crossref_articles(keywords):
	...
	query_Crossref = "+".join(keywords)	# Crossref does not allow the use of operator "AND"
	url_Crossref = base_url_Crossref + query_Crossref + "&sort=relevance" + "&rows=1000"
	...
	Crossref_articles = [process_Crossref_article(item) for item in Crossref_items]
...
# performs a single API call to CORE
def call_CORE_api(page_CORE, query_CORE, api_key_CORE):
	url_CORE = base_url_CORE + query_CORE + "?page=" + str(page_CORE) + "&pageSize=" + str(100) + "&metadata=true&fulltext=false&citations=false&similar=false&duplicate=false&urls=false&faithfulMetadata=false&apiKey=" + api_key_CORE
...
# task to retrieve articles from CORE in json format
...
def get_CORE_articles(keywords):
	...
	# since CORE returns only 100 results per page, we have to make 10 calls to get 1000 results
	CORE_list_of_lists = [call_CORE_api(page_CORE, query_CORE, api_key_CORE) for page_CORE in range(1,11)]

	CORE_list = [item for sublist in CORE_list_of_lists for item in sublist if sublist]
...
def get_claims(abstract):
	...
	# for now, we only take sentences that have claim words (not those before or after)
	# this may change later 
	claims_list = []
	for sentence in abstract_sentences:
		for word in claim_words:
			if word + ' that' in sentence or word + ' to be' in sentence or word in sentence:
...
# Filter out articles whose abstracts do not have all the keywords or make any claim
def filter_articles(articles, keywords):
	relevant_articles = []

	articles = [article for article in articles if not missing_key_words(article['abstract'], keywords)]
	
	for article in articles:
		abstract_sentences = sent_tokenize(article["abstract"])	# this is a list
...
# use the subjects, verbs, and objects in a sentence to determine the claim position
def use_subjects_verbs_objects(sent_list, keywords):
	...
	while i != list_length:
		...
		for triple in svo_extract:
			...
			elif 'not' in verb.text and (obj.text in keywords or obj.lemma_ in keywords):
				if position != False:
					position = False
...
def get_dependency_path_between_tokens(token_a, token_b):
	if token_b in token_a.subtree:
		...
		tokens_a_side = get_dependency_path_to_root(token_a, lowest_common_ancestor)
		tokens_b_side = get_dependency_path_to_root(token_b, lowest_common_ancestor)
...
def use_noun_phrases(sent_list, keywords):
	...
	while i != list_length:
		...
		for np in noun_phrases:
			...
			for word in words:
				if word in keywords:
					...
					# if it contains more than one key word otherwise big trouble later
					break
		
		# use the dependency path between noun phrases (if we have only two noun phrases)
		if len(relevant_noun_phrases) == 2:
			tokens_btn_nps = get_dependency_path_between_tokens(relevant_noun_phrases[0], relevant_noun_phrases[1])
		...
		# in case there is only one or more than two noun phrases
			...
			# we do not want position to change from false to 'not enough relevant noun phrases'
			if position == False:
				position = False
...
def use_effect_words(sent_list, keywords):
	...
	for sent in sent_list:
		for word in keywords:
			if word in sent:
				if 'positive' in sent and ('effects' in sent or 'effect' in sent \
...
def determine_claim_position(sent_list, keywords):
	...
	if sent_list:
		...
		# this really only gets returned if we use only claim statements and an abstract happens to
		# have an empty list of claims 
		return position
...
def assign_position(relevant_articles, keywords):
	...
	for item in relevant_articles:
		...
		# the claim position is the position determined by examining sentences which make explicit claims 
		if determine_claim_position(claims, keywords) == True:
			item['claim_position'] = 'Yes'
		elif determine_claim_position(claims, keywords) == False:
		...
		# the total position includes all sentences in the abstract and is therefore a superset 
		# of the claim position
		if determine_claim_position(abstract_sentences, keywords) == True:
			item['total_position'] = 'Yes'
		elif determine_claim_position(abstract_sentences, keywords) == False:
...
def get_article_positions(relevant_articles):
	claim_positions = {"Yes":0, "No":0, "No_position":0, "Yes_percent":0, "No_percent":0, "No_position_percent":0}
	...
	No_position_percent = round((claim_positions["No_position"]/all_articles)*100, 2)
	...
	detailed_recommendation = "Of the {} relevant open access research articles we surveyed, {}% of them leaned towards a {} to your \
	question, {}% leaned towards a {}, and {}% did not state an explicit position.".format(all_articles, Yes_percent, "Yes", No_percent, "No", No_position_percent) \
...
# task to perform claim detection on abstracts
...
def answer_question(articles, keywords=None):
	...
	combined_articles = [item for sublist in articles for item in sublist if item is not None]
	...
	if len(combined_articles) != 0:
		...
		if len(relevant_articles) != 0:
			...
			relevant_articles_positions = assign_position(relevant_articles, keywords)
			# get summary of claim positions
			claim_positions = get_article_positions(relevant_articles_positions)
			...
			# generate recommendation to give to user
			detailed_recommendation = "Unfortunately, none of the open access research articles we surveyed were relevant to your query. We are, \
			therefore, unable to return a recommendation."
		...
		# generate recommendation to give to user
		detailed_recommendation = "Unfortunately, we were not able to get any open access research articles related to your query. We are, \
		therefore, unable to return a recommendation."
...
if __name__ == '__main__':
	...
	# use a celery chord here and add keywords as an extra argument in addition to the header
